...
O visual do projeto é um dark UI semântico: fundo escuro neutro, superfícies em camadas e tokens de cor previsíveis. A implementação atual usa `#121212` como base e constrói elevação com classes nativas do Tailwind, sem depender de utilitários customizados de sombra ou borda.

O objetivo é parecer estável, limpo e útil, com contraste suficiente para leitura e profundidade visual obtida por `shadow-*`, `ring-*` e bordas sutis.

Inter continua sendo a fonte principal. O uso de OpenType features e tracking positivo permanece válido quando melhora a leitura em fundo escuro.

**Key Characteristics:**

- Dark base background (`#121212`) with semantic surfaces acima dela
- Elevação construída com `shadow-sm`, `ring-1`, `ring-inset` e bordas de baixa opacidade
- Acentos de produto via `--color-dev` e `--color-editor`
- Inter com tracking positivo e OpenType features para leitura em dark UI
- Borda sutil e consistência visual por tokens semânticos
- Componentes interativos com feedback por opacidade e motion, não por ruído visual
